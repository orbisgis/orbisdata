...
class GroovyH2GISTest {
    ...
    @Test
    void openH2GIS() {
        assertNotNull org.orbisgis.data.jdbc.h2gis.H2GIS.open("./target/openH2GIS1")
        assertNotNull org.orbisgis.data.jdbc.h2gis.H2GIS.open("./target/openH2GIS2", "sa", "sa")
    }

    @Test
    void testColumnsType() throws SQLException {
        def h2GIS = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/loadH2GIS'])
        assertNotNull h2GIS
        def name = "TYPES"
    ...
    void loadH2GIS() {
        ...
        org.orbisgis.data.jdbc.h2gis.H2GIS datasource = org.orbisgis.data.jdbc.h2gis.H2GIS.open(directory.absolutePath + File.separator + "osm_chain_db;AUTO_SERVER=TRUE")
    ...
    void loadH2GISFromFile() {
        ...
        def h2GIS = org.orbisgis.data.jdbc.h2gis.H2GIS.open(new File('target/config.properties'))
        assertNotNull(h2GIS)
    }
    ...
    void loadExternalTableFromDataBase() {
        def h2External = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/secondH2GIS', user: 'sa', password: 'sa'])
        h2External.execute("""
    ...
    void linkExternalFile() {
        def h2GIS = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/secondH2GIS', user: 'sa', password: 'sa'])
        h2GIS.execute("""
                DROP TABLE IF EXISTS externalTable;
                CREATE TABLE externalTable (id int, the_geom geometry(point));
                INSERT INTO externalTable VALUES (1, 'POINT(10 10)'::GEOMETRY), (2, 'POINT(1 1)'::GEOMETRY);
    ...
    @Test
    void importOSMFile() {
        org.orbisgis.data.jdbc.h2gis.H2GIS h2GIS = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/loadH2GIS'])
    ...
    @Test
    void testColumn() {
        def h2GIS = org.orbisgis.data.jdbc.h2gis.H2GIS.open('./target/orbisgis')
        h2GIS.execute("""
    ...
    void saveTableToPOSTGIS() {
        ...
        def postGIS = org.orbisgis.data.jdbc.postgis.POSTGIS.open(dbProperties)
        if(postGIS) {
    ...
    void saveTableToH2GIS() {
        ...
        h2GIS.execute("""
                ...
                INSERT INTO h2gis VALUES (1, 'POINT(10 10)'::GEOMETRY), (2, 'POINT(1 1)'::GEOMETRY);
        """)
        def h2GISTarget = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/loadH2GIS_target'])
