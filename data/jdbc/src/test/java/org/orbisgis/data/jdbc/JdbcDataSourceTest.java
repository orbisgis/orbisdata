...
import org.orbisgis.data.jdbc.h2gis.H2gisSpatialTable;
import org.orbisgis.data.jdbc.postgis.POSTGIS;
import org.orbisgis.data.jdbc.postgis.PostgisSpatialTable;
...
class JdbcDataSourceTest {
    ...
    void testGetDataBaseType() {
        Assertions.assertEquals(DBTypes.H2GIS, h2gis.getDataBaseType());
        Assertions.assertEquals(DBTypes.POSTGIS, postgis.getDataBaseType());
    }
    ...
    void testExecute() throws SQLException {
        ...
        Assertions.assertTrue(h2gis.execute(gstring1.toString()));
        Assertions.assertFalse(h2gis.execute(gstring2.toString()));
        Assertions.assertTrue(h2gis.execute(gstring3.toString()));
        ...
        Assertions.assertTrue(postgis.execute(gstring1.toString()));
        Assertions.assertFalse(postgis.execute(gstring2.toString()));
        Assertions.assertTrue(postgis.execute(gstring3.toString()));
    }
    ...
    void testFirstRow() throws SQLException {
        String gstring1_h2gis = "SELECT * FROM test_h2gis";
        String gstring1_postgis = "SELECT * FROM test_postgis";
        Assertions.assertFalse(h2gis.firstRow(gstring1_h2gis).isEmpty());
        Assertions.assertEquals("{ID=1, THE_GEOM=POINT (0 0), TEXT=toto}", h2gis.firstRow(gstring1_h2gis).toString());
        Assertions.assertFalse(postgis.firstRow(gstring1_postgis.toString()).isEmpty());
        Assertions.assertEquals("{id=1, the_geom=POINT (0 0), text=toto}", postgis.firstRow(gstring1_postgis).toString());
  }
    ...
    void testExecuteMethod1() throws URISyntaxException, SQLException {
        ...
        Assertions.assertTrue(h2gis.executeScript(file.getAbsolutePath(), map));
        Assertions.assertFalse(h2gis.executeScript("toto", map));
        String str = h2gis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{ID=1}\n{ID=11}\n{ID=51}", str);
        Assertions.assertTrue(h2gis.executeScript(url.toURI().toString(), map));
        str = h2gis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{ID=1}\n{ID=11}\n{ID=51}", str);
        Assertions.assertTrue(h2gis.executeScript(url.toString(), map));
        str = h2gis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{ID=1}\n{ID=11}\n{ID=51}", str);

        Assertions.assertTrue(postgis.executeScript(file.getAbsolutePath(), map));
        Assertions.assertFalse(postgis.executeScript("toto", map));
        str = postgis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{id=1}\n{id=11}\n{id=51}", str);
        Assertions.assertTrue(postgis.executeScript(url.toURI().toString(), map));
        str = postgis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{id=1}\n{id=11}\n{id=51}", str);
        Assertions.assertTrue(postgis.executeScript(url.toString(), map));
        str = postgis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{id=1}\n{id=11}\n{id=51}", str);

        Assertions.assertFalse(h2gis.executeScript("notAFile.sql"));
        Assertions.assertFalse(postgis.executeScript("notAFile.sql"));
    }
    ...
    void testExecuteMethod2() throws SQLException {
        ...
        Assertions.assertTrue(h2gis.executeScript(this.getClass().getResourceAsStream("simpleWithArgs.sql"), map));
        String str = h2gis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{ID=1}\n{ID=11}\n{ID=51}", str);

        Assertions.assertTrue(postgis.executeScript(this.getClass().getResourceAsStream("simpleWithArgs.sql"), map));
        str = postgis.rows("SELECT * FROM script").stream().map(Objects::toString).collect(Collectors.joining("\n"));
        assertEquals("{id=1}\n{id=11}\n{id=51}", str);

        Assertions.assertFalse(h2gis.executeScript(this.getClass().getResourceAsStream("badSql.sql"), map));
        Assertions.assertFalse(postgis.executeScript(this.getClass().getResourceAsStream("badSql.sql"), map));
    ...
    void testMetaClassMethods() {
        Assertions.assertEquals(InvokerHelper.getMetaClass(H2GIS.class), h2gis.getMetaClass());
        h2gis.setMetaClass(InvokerHelper.getMetaClass(this));
        Assertions.assertEquals(InvokerHelper.getMetaClass(this), h2gis.getMetaClass());

        Assertions.assertEquals(InvokerHelper.getMetaClass(POSTGIS.class), postgis.getMetaClass());
        postgis.setMetaClass(InvokerHelper.getMetaClass(this));
        Assertions.assertEquals(InvokerHelper.getMetaClass(this), postgis.getMetaClass());
    }
    ...
    void testGetLocation() {
        ...
        Assertions.assertEquals(DB_NAME, h2gis.getLocation().toString());
        assertNotNull(postgis.getLocation());
        Assertions.assertTrue(postgis.getLocation().toString().startsWith("5432/"));
    }
