...
class GroovyPostGISTest {
    ...
    static org.orbisgis.data.jdbc.postgis.POSTGIS postGIS;


    @BeforeAll
    static void init() {
        postGIS = org.orbisgis.data.jdbc.postgis.POSTGIS.open(dbProperties)
    ...
    void loadTableFromH2GISToPostGIS() {
        def h2GISSource = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/loadH2GIS_source'])
        h2GISSource.execute("""
    ...
    void saveH2GISTableToPostGIS() {
        def h2GIS = org.orbisgis.data.jdbc.h2gis.H2GIS.open([databaseName: './target/loadH2GIS_source'])
        h2GIS.execute("""
                DROP TABLE IF EXISTS h2gis;
    ...
    void linkTableFromPostGISToH2GIS() {
        ...
        def h2GISSource = org.orbisgis.data.jdbc.h2gis.H2GIS.open("./target/linkedTable")
