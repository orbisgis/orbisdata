...
public abstract class JdbcDataSource extends Sql implements IJdbcDataSource, IResultSetBuilder {
    ...
    public JdbcDataSource(DataSource dataSource, DBTypes databaseType) {
        ...
        try {
            ...
            LOGGER.warn("Unable to set the autocommit option of the connection to false.");
    ...
    public Collection<String> getTableNames(String catalogPattern, String schemaPattern,
        ...
        try {
            ...
            LOGGER.error("Unable to get the table names.");
            LOGGER.debug(e.getLocalizedMessage());
        }
    ...
    public void eachRow(String sql,
        ...
        try {
            ...
            LOGGER.debug("Unable to execute the request as a String.",e.getLocalizedMessage());
            try {
                if(!getConnection().getAutoCommit()){
                    super.rollback();
                }
    ...
    public void eachRow(GString gstring,
        ...
        try {
            ...
            LOGGER.error("Unable to execute the request as a GString." );
            LOGGER.debug(e.getLocalizedMessage());
            try {
                if(!getConnection().getAutoCommit()){
                    super.rollback();
    ...
    @Override
    public Map<String, String> getIndexes(String tableName) throws SQLException {
        return JDBCUtilities.getIndexNames(getConnection(), TableLocation.parse(tableName, getDataBaseType()));
    }

    @Override
    public List<String> getIndexes(String tableName, String columnName) throws SQLException {
        return JDBCUtilities.getIndexNames(getConnection(), TableLocation.parse(tableName, getDataBaseType()), columnName);
    }

    @Override
    public void createIndex(String tableName, String columnName) throws SQLException {
        JDBCUtilities.createIndex(getConnection(), TableLocation.parse(tableName, getDataBaseType()), columnName);
    }
    ...
    public Collection<String> getColumnNames(String tableName){
        try {
            Collection<String> cols = JDBCUtilities.getColumnNames(getConnection(), TableLocation.parse(tableName, getDataBaseType()));
            return cols;
        } catch (SQLException e) {
            LOGGER.error("Unable to get the column names of the table " + tableName + ".");
            LOGGER.debug(e.getLocalizedMessage());
            return null;
        }
    }
